.global s_prepare_buffers
s_prepare_buffers:
push %r10
push %r11
push %r12
push %r15
push %rbx
push %rcx
push %rdi
push %rsi
lea addresses_normal_ht+0xf238, %r10
nop
nop
nop
cmp $35003, %r12
mov (%r10), %r11w
nop
nop
and $41232, %r15
lea addresses_D_ht+0x1dd42, %r15
nop
xor %r10, %r10
mov $0x6162636465666768, %rbx
movq %rbx, %xmm1
movups %xmm1, (%r15)
nop
and $8970, %r11
lea addresses_D_ht+0x10202, %rsi
lea addresses_A_ht+0x18812, %rdi
nop
nop
nop
xor %r10, %r10
mov $27, %rcx
rep movsl
nop
nop
xor %r11, %r11
lea addresses_A_ht+0xb632, %rsi
lea addresses_A_ht+0x9a02, %rdi
nop
nop
nop
nop
xor $42036, %r15
mov $32, %rcx
rep movsb
nop
nop
nop
nop
dec %rbx
lea addresses_WC_ht+0x10602, %rdi
nop
sub %rbx, %rbx
mov (%rdi), %r11
nop
nop
nop
nop
nop
and %rcx, %rcx
lea addresses_UC_ht+0xa458, %r15
nop
nop
nop
nop
dec %rbx
movb $0x61, (%r15)
nop
nop
nop
add %rsi, %rsi
lea addresses_A_ht+0xfa02, %r15
nop
nop
nop
nop
xor %r10, %r10
movb (%r15), %r12b
nop
cmp $24372, %rbx
lea addresses_D_ht+0x8ea2, %rdi
nop
nop
nop
nop
nop
and $50342, %rcx
mov (%rdi), %r10
nop
nop
nop
nop
nop
sub $61259, %rdi
lea addresses_WT_ht+0x156a2, %r15
nop
nop
nop
nop
and %rsi, %rsi
mov $0x6162636465666768, %r10
movq %r10, %xmm7
movups %xmm7, (%r15)
nop
nop
nop
cmp %r12, %r12
lea addresses_normal_ht+0x17e05, %rsi
lea addresses_UC_ht+0xe582, %rdi
nop
nop
nop
nop
nop
dec %r11
mov $31, %rcx
rep movsl
nop
nop
sub %r10, %r10
lea addresses_UC_ht+0xf41d, %rdi
nop
nop
nop
nop
nop
xor $13086, %rcx
mov (%rdi), %ebx
nop
nop
nop
and $37498, %r12
lea addresses_normal_ht+0x1ed02, %rsi
lea addresses_WC_ht+0x4202, %rdi
nop
nop
nop
nop
nop
xor $14133, %rbx
mov $54, %rcx
rep movsb
nop
nop
dec %rbx
pop %rsi
pop %rdi
pop %rcx
pop %rbx
pop %r15
pop %r12
pop %r11
pop %r10
ret

    .global s_faulty_load
s_faulty_load:
push %r10
push %r14
push %r15
push %rbx
push %rcx
push %rdx

// Store
lea addresses_UC+0x12b6a, %rdx
nop
nop
nop
xor $64819, %rbx
movl $0x51525354, (%rdx)
nop
nop
nop
nop
nop
add $10659, %rdx

// Faulty Load
lea addresses_normal+0x1c202, %rbx
dec %r10
movups (%rbx), %xmm0
vpextrq $0, %xmm0, %r14
lea oracles, %r15
and $0xff, %r14
shlq $12, %r14
mov (%r15,%r14,1), %r14
pop %rdx
pop %rcx
pop %rbx
pop %r15
pop %r14
pop %r10
ret

/*
<gen_faulty_load>
[REF]
{'OP': 'LOAD', 'src': {'type': 'addresses_normal', 'size': 8, 'AVXalign': False, 'NT': True, 'congruent': 0, 'same': False}}
{'OP': 'STOR', 'dst': {'type': 'addresses_UC', 'size': 4, 'AVXalign': False, 'NT': False, 'congruent': 3, 'same': False}}
[Faulty Load]
{'OP': 'LOAD', 'src': {'type': 'addresses_normal', 'size': 16, 'AVXalign': False, 'NT': False, 'congruent': 0, 'same': True}}
<gen_prepare_buffer>
{'OP': 'LOAD', 'src': {'type': 'addresses_normal_ht', 'size': 2, 'AVXalign': False, 'NT': False, 'congruent': 1, 'same': False}}
{'OP': 'STOR', 'dst': {'type': 'addresses_D_ht', 'size': 16, 'AVXalign': False, 'NT': False, 'congruent': 6, 'same': False}}
{'OP': 'REPM', 'src': {'type': 'addresses_D_ht', 'congruent': 10, 'same': False}, 'dst': {'type': 'addresses_A_ht', 'congruent': 4, 'same': False}}
{'OP': 'REPM', 'src': {'type': 'addresses_A_ht', 'congruent': 4, 'same': False}, 'dst': {'type': 'addresses_A_ht', 'congruent': 11, 'same': False}}
{'OP': 'LOAD', 'src': {'type': 'addresses_WC_ht', 'size': 8, 'AVXalign': False, 'NT': False, 'congruent': 9, 'same': False}}
{'OP': 'STOR', 'dst': {'type': 'addresses_UC_ht', 'size': 1, 'AVXalign': False, 'NT': False, 'congruent': 1, 'same': False}}
{'OP': 'LOAD', 'src': {'type': 'addresses_A_ht', 'size': 1, 'AVXalign': False, 'NT': False, 'congruent': 10, 'same': False}}
{'OP': 'LOAD', 'src': {'type': 'addresses_D_ht', 'size': 8, 'AVXalign': False, 'NT': False, 'congruent': 4, 'same': False}}
{'OP': 'STOR', 'dst': {'type': 'addresses_WT_ht', 'size': 16, 'AVXalign': False, 'NT': False, 'congruent': 5, 'same': False}}
{'OP': 'REPM', 'src': {'type': 'addresses_normal_ht', 'congruent': 0, 'same': False}, 'dst': {'type': 'addresses_UC_ht', 'congruent': 6, 'same': False}}
{'OP': 'LOAD', 'src': {'type': 'addresses_UC_ht', 'size': 4, 'AVXalign': False, 'NT': False, 'congruent': 0, 'same': False}}
{'OP': 'REPM', 'src': {'type': 'addresses_normal_ht', 'congruent': 8, 'same': False}, 'dst': {'type': 'addresses_WC_ht', 'congruent': 11, 'same': False}}
{'34': 140}
34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34 34
*/
